  ...
  <div
    ...
    className={
      `group relative flex flex-col overflow-hidden rounded-[24px] p-6 w-full cursor-pointer
       border border-white/10 bg-white/[0.04] backdrop-blur-xl shadow-[0_0_40px_var(--color-cyan-shadow,0.08)]
       transition duration-300
       ${disabled ? 'opacity-50 pointer-events-none' : 'hover:-translate-y-1 hover:rotate-[0.6deg] hover:border-white/20 hover:shadow-[0_0_60px_var(--color-cyan-shadow,0.18)]'}`
    }
  >
    ...
    <div className="flex items-start justify-between gap-2">
      <h3 className="text-xl text-[var(--color-cyan)] mb-2">{title}</h3>
      {(demoVideo || demoImages) && (
        <span className="shrink-0 mt-1 text-[var(--color-green)] opacity-70 group-hover:opacity-100 transition-opacity" title={demoVideo ? "Demo video available" : "Demo screenshots available"}>
